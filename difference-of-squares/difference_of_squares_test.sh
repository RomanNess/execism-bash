...
@test "print usage if wrong number of arguments provided" {
  run bash difference_of_squares.sh difference 1 2

  [[ "$status" -eq 1 ]]
  [[ "$output" == "Usage: difference_of_squares.sh <square_of_sum|sum_of_squares|difference> <integer>" ]]
}

@test "print usage if wrong first argument provided" {
  run bash difference_of_squares.sh sum 1
