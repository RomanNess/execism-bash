...
@test 'invalid characters lead to invalid isbn' {
  run bash isbn_verifier.sh '3_59821507X'
  [ "$status" -eq 0 ]
  [ "$output" == 'false' ]
}

@test 'all hyphens are ignored' {
  run bash isbn_verifier.sh '3-5-9-8-2-1-5-0-7-X'
  [ "$status" -eq 0 ]
  [ "$output" == 'true' ]
}
