...
@test 'Print usage if no argument provided' {
  run bash armstrong_numbers.sh

  [[ "$status" -eq 1 ]]
  [[ "$output" = "Usage: armstrong_numbers.sh <integer>" ]]
}

@test 'Print usage if too many arguments provided' {
  run bash armstrong_numbers.sh 1 2
